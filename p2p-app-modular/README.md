# P2P Manager — Procure-to-Pay Web App (Modular Edition)
...
## Modules (license gating)

The **core** (auth + 2FA, users, departments, approval matrix, vendors + KYC,
PR → PO → GRN, invoices + 3-way match, My Approvals, dashboard, notifications)
is always on. Everything else is a module switched by the `MODULES` env var —
effectively the commercial license key per client:

| Module | Adds | Enable with |
|---|---|---|
| `tax` | India tax & accounting: multi-GSTIN, TDS/WHT at approval (incl. lower-deduction certificates), RCM, journal entries + CSV/Excel export, vendor statements, TDS/RCM deposits, GSTR-2B recon, GL account codes/classification fields for external accounting-system integration (Tally/SUN6/Dynamics/...) | `MODULES=tax` |
| `payments` | Maker-checker payments, bank bulk-upload CSV, payment advices | `MODULES=payments` |
| `vendor_portal` | Supplier self-service portal at `/vendor` — can also run as its **own container** (`npm run start:vendor`) so only it faces the internet | `MODULES=vendor_portal` |
| `inventory` | *Planned* — database schema is already in place (`items`, `warehouses`, `stock_ledger`, plus `item_id` on PR/PO lines), so enabling it later is a config change, not a migration | — |

`MODULES` unset or `all` → every shipped module; `MODULES=tax,payments` → just
those; `MODULES=` (empty) → core only. A disabled module's routes are never
mounted (its APIs return 404) and its nav items are hidden — the frontend
learns the enabled list from the login response. Every module's tables always
exist in the schema, so upselling a module later needs no data migration.

Without `tax`, invoices are approved without TDS or journal entries (GST
amounts are still recorded as entered) and POs/invoices don't require a GSTIN —
for clients who do their accounting in their own ERP/Tally. Without
`payments`, the workflow ends at invoice approval.
...
```bash
# one-time: local PostgreSQL for development (container shared with the classic
# app; this edition uses its own databases p2p_mod / p2p_mod_test)
docker run -d --name p2p-postgres --restart unless-stopped \
  -e POSTGRES_USER=p2p -e POSTGRES_PASSWORD=p2p -e POSTGRES_DB=p2p \
  -p 5433:5432 postgres:16-alpine
docker exec p2p-postgres psql -U p2p -d p2p -c "CREATE DATABASE p2p_mod"
docker exec p2p-postgres psql -U p2p -d p2p -c "CREATE DATABASE p2p_mod_test"

npm install
npm start                # staff app: http://localhost:9139 · vendor portal: http://localhost:9139/vendor
MODULES=tax npm start    # core + tax only (no payments, no vendor portal)
npm run start:vendor     # standalone vendor portal on http://localhost:9140/vendor
```

`DATABASE_URL` defaults to `postgres://p2p:p2p@localhost:5433/p2p_mod` for development.
`npm run dev` restarts on file changes; `npm run seed` wipes the database and reseeds
demo data; `npm test` runs the API test suite (uses a separate `p2p_mod_test` database),
including a module-gating test that boots a core-only instance.
Set `JWT_SECRET` in production (otherwise a random secret is generated per restart, which
invalidates sessions on restart). `PORT` overrides the default 9139.
...
### GL / external accounting integration (`tax` module)
...
- **Invoice Receipt Date** (separate from the vendor's invoice date; captured
  regardless of the `tax` module) drives **due date auto-calculation**:
  receipt date + the vendor's payment-terms days (`payment_terms_days`,
  editable per vendor). An explicit due date always overrides the calculation.
...
- `GET|POST /vendors/:id/tds-certificates`, `PUT /vendors/:id/tds-certificates/:certId`, `GET .../:certId/file` (`tax` module)
...
- `GET|POST /settings/ap-account-codes|sub-locations|cost-centres`, `PUT .../:id` (admin, `tax` module)
- `GET|PUT /settings/custom-field-labels` (admin, `tax` module)
