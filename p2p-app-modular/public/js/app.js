...
// licensed modules — the server advertises them on login / /auth/me
let MODULES = JSON.parse(localStorage.getItem('p2p_modules') || '[]');
const has = (mod) => MODULES.includes(mod);
function setModules(list) {
  if (!Array.isArray(list)) return;
  MODULES = list;
  localStorage.setItem('p2p_modules', JSON.stringify(list));
}
...
function logout() {
  ...
  localStorage.removeItem('p2p_modules');
  showLogin();
}
