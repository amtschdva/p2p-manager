...
const TEST_DB = process.env.TEST_DATABASE_URL || 'postgres://p2p:p2p@localhost:5433/p2p_mod_test';
...
test('module gating: a core-only server hides tax, payments and vendor portal', async () => {
  ...
  const gateBase = `http://localhost:${gatePort}`;
  const gated = spawn(process.execPath, ['src/server.js'], {
    cwd: APP_ROOT,
    env: { ...process.env, PORT: String(gatePort), P2P_DATA_DIR: tmpDir, DATABASE_URL: TEST_DB,
           JWT_SECRET: 'test-secret-for-suite', NODE_ENV: 'development', MODULES: '' },
    stdio: ['ignore', 'pipe', 'pipe'],
  });
  try {
    let up = false;
    for (let i = 0; i < 40 && !up; i++) {
      try { up = (await fetch(`${gateBase}/api/auth/me`)).status === 401; }
      catch { await new Promise((r) => setTimeout(r, 500)); }
    }
    assert.ok(up, 'core-only server started');

    const loginRes = await fetch(`${gateBase}/api/auth/login`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ username: 'admin', password: 'admin123' }),
    });
    const loginJson = await loginRes.json();
    assert.equal(loginRes.status, 200);
    assert.deepEqual(loginJson.modules, [], 'login advertises no optional modules');
    const tok = loginJson.token;

    // module routes are simply not mounted
    for (const gone of ['/api/journal', '/api/payments', '/api/tax/summary', '/api/settings/gstins']) {
      const r = await fetch(`${gateBase}${gone}`, { headers: { Authorization: 'Bearer ' + tok } });
      assert.equal(r.status, 404, `${gone} should 404 on a core-only install`);
    }
    const vendorLogin = await fetch(`${gateBase}/api/vendor/login`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: 'vendor@techsupply.in', password: 'vendor123' }),
    });
    assert.equal(vendorLogin.status, 404, 'vendor portal API off');

    // core still fully works (and tax masters come back empty, not an error)
    const meta = await fetch(`${gateBase}/api/meta/tax`, { headers: { Authorization: 'Bearer ' + tok } });
    const metaJson = await meta.json();
    assert.equal(meta.status, 200);
    assert.deepEqual(metaJson.gstins, []);
    assert.ok(metaJson.departments.length > 0, 'departments (core) still served');
  } finally {
    gated.kill();
  }
});
